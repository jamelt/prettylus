...
- Refactor extension structure for better maintainability and testability
- Add dedicated modules for configuration loading, Prettier and Stylus Supremacy module loading, and logging
- Update build process to use `dist/` instead of `out/` for output files
- Add `prettylus.prettierBaseDirectory` setting to specify base directory for Prettier config resolution and plugin loading
- Add `prettylus.prettierConfigFile` setting to specify an explicit Prettier config file
- Enhance logging with dedicated logger class and output channel for debugging
- Add install script for VSIX package installation

## 0.3.3

- Update dependencies and configuration for Prettier 3.2.0
- Switch from yarn to npm (package-lock.json)
